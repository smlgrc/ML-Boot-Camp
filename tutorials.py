...
class DeepNeuralNetwork():
    ...
    def softmax(self, x, derivative=False):
        ...
        exps = np.exp(x - x.max())
        softmax_func = exps / np.sum(exps, axis=0)
        if derivative:
            # return softmax_func * (1 - softmax_func)
    ...
    def compute_accuracy(self, x_val, y_val):
        ...
        for x, y in zip(x_val, y_val):
            ...
            output = self.forward_pass(y)
            pred = np.argmax(output)
            predictions.append(pred == np.argmax(y))

        return np.mean(predictions)

    def train(self, x_train: pd.DataFrame, y_train: np.ndarray, x_val: pd.DataFrame, y_val: np.ndarray):
        ...
        for iteration in range(self.epochs):
            for x, y in zip(x_train, y_train):
                ...
                output = self.forward_pass(x_train[x])
